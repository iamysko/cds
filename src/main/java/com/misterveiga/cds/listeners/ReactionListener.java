			...
			message.getJDA().getGuildById(message.getGuild().getIdLong())
					.retrieveMemberById(message.getAuthor().getId()).queue(messageAuthor -> {
						...
						case ID_REACTION_QM_30:
							...
							System.out.println("isStaffOnStaff: " + isStaffOnStaff(reactee, messageAuthor, commandChannel))
							System.out.println("isInStaffChannel: " + isInStaffChannel(reactee, commandChannel, event.getChannel()))
							System.out.println("isAnyRole: " + RoleUtils.isAnyRole(reactee, RoleUtils.ROLE_SERVER_MANAGER, RoleUtils.ROLE_COMMUNITY_SUPERVISOR, RoleUtils.ROLE_BOT))
